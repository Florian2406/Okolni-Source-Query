...
namespace Okolni.Source.Query.Common
{
    internal static class QueryHelper
    {
        ...
        private static async Task Request(byte[] requestMessage, IPEndPoint endPoint, Socket socket, int SendTimeout)
        {
        ...
        private static async Task<byte[]> ReceiveAsync(IPEndPoint endPoint, Socket socket, int ReceiveTimeout)
        {
            var newCancellationToken = new CancellationTokenSource();
            ...
            try
            {
                ...
                var udpClientReceive = await socket.ReceiveFromAsync(buffer, SocketFlags.None, endPoint, newCancellationToken.Token);
                var recvPacket = buffer.Slice(0, udpClientReceive.ReceivedBytes);
        ...
        private static async Task<byte[]> FetchResponse(IPEndPoint endPoint, Socket socket, int ReceiveTimeout)
        {
        ...
        private static async Task<byte[]> FetchMultiPacketResponse(IByteReader byteReader, IPEndPoint endPoint, Socket socket, int ReceiveTimeout)
        {
        ...
        public static async Task<InfoResponse> GetInfoAsync(IPEndPoint endPoint, Socket socket, int sendTimeout, int receiveTimeout, int maxRetries)
        {
            try
            {
                var requestData = await RequestDataFromServer(Constants.A2S_INFO_REQUEST, endPoint, socket, maxRetries, sendTimeout, receiveTimeout);
        ...
        /// <exception cref="SourceQueryException"></exception>
        /// 
        public static async Task<PlayerResponse> GetPlayersAsync(IPEndPoint endPoint, Socket socket, int sendTimeout, int ReceiveTimeout, int maxRetries = 10)
        {
            try
            {
                var requestData = await RequestDataFromServer(Constants.A2S_PLAYER_CHALLENGE_REQUEST, endPoint, socket, maxRetries, sendTimeout, ReceiveTimeout, true);
        ...
        /// <exception cref="SourceQueryException"></exception>
        public static async Task<RuleResponse> GetRulesAsync(IPEndPoint endPoint, Socket socket, int sendTimeout, int receiveTimeout, int maxRetries = 10)
        {
            try
            {
                var requestData = await RequestDataFromServer(Constants.A2S_RULES_CHALLENGE_REQUEST, endPoint, socket, maxRetries, sendTimeout, receiveTimeout, true);
        ...
        public static async Task<(IByteReader reader, byte header)> RequestDataFromServer(byte[] request, IPEndPoint endPoint, Socket socket, int maxRetries, int sendTimeout, int ReceiveTimeout, bool replaceLastBytesInRequest = false)
        {
            int retries = 0;
